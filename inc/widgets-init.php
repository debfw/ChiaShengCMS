 ...
 * Base Widget Class
 */
class ChiaSheng_Widget_Base extends WP_Widget
{
    /**
     * @var Array of string
     */
     ...
     */
    public function ChiaSheng_generate_text_input($field, $label, $value, $type = 'text', $note = '', $class = '')
    {
     ...
     * @param string $note field note to appear below
     */
    public function ChiaSheng_generate_textarea($field, $label, $note = '')
    {
     ...
     */
    public function ChiaSheng_generate_checkbox_input($field, $label, $elements, $note = '')
    {
        $instance = isset($this->form_instance[$field]) ? $this->form_instance[$field] : true;
        ?>
        <div class="ChiaSheng-multiple-check-form">
            <p>
                <label for="<?php echo esc_attr($this->get_field_id($field)); ?>">
                    <?php echo esc_html($label); ?>
                </label>
     ...
     * @param Array_A $elements
     */
    public function ChiaSheng_generate_select_options($field, $label, $elements, $note = '')
    {
    ...
    public function chiaSheng_generate_image_upload($field, $label, $value, $note = '', $class = '')
    {
        $instance = isset($this->form_instance[$field]) ? $this->form_instance[$field] : '';
        ?>
        <div>
            ...
            <div class="image-preview-wrap">
                ...
                <input type="button" class="select-img button button-primary"
                       value="<?php esc_attr_e('Upload', 'chiaSheng'); ?>"
                       data-uploader_title="<?php esc_attr_e('Select Image', 'chiaSheng'); ?>"
                       data-uploader_button_text="<?php esc_attr_e('Choose Image', 'chiaSheng'); ?>"/>
                <?php
                $image_status = false;
                if (!empty($instance)) {
                    $image_status = true;
                ...
                ?>
                <input type="button" value="<?php echo _x('X', 'Remove', 'chiaSheng'); ?>"
    ...
    public function chiaSheng_generate_checkbox_options($field, $label, $elements, $note = '')
    {
        $instance = isset($this->form_instance[$field]) ? $this->form_instance[$field] : $label;
        $count_per_row = isset($this->form_instance['']) ?
        $this->form_instance[''] :
            $this->form_instance[''];
        ?>
        <p>
    ...
    {
        $instance = array();
        $instance = $this->chiaSheng_sanitize_data($instance, $new_instance);
        return $instance;
    }

    public function chiaSheng_sanitize_data($instance, $new_instance)
    {
        if (is_array($this->text_fields)) {
            // update the text fields values
            foreach ($this->text_fields as $field) {
                $instance = array_merge($instance, $this->chiaSheng_update_text($field, $new_instance));
            }
        }
        ...
        if (is_array($this->url_fields)) {
            ...
            foreach ($this->url_fields as $field) {
                $instance = array_merge($instance, $this->chiaSheng_update_url($field, $new_instance));
            }
        }
        ...
        if (is_array($this->text_areas)) {
            ...
            foreach ($this->text_areas as $field) {
                $instance = array_merge($instance, $this->chiaSheng_update_textarea($field, $new_instance));
            }
        }
        if (is_array($this->checkboxes)) {
            // update the checkbox fields values
            foreach ($this->checkboxes as $field) {
                $instance = array_merge($instance, $this->chiaSheng_update_checkbox($field, $new_instance));
            }
        }
        if (is_array($this->select_fields)) {
            // update the select fields values
            foreach ($this->select_fields as $field) {
                $instance = array_merge($instance, $this->chiaSheng_update_select($field, $new_instance));
            }
        }
        return $instance;
    }
     ...
     */
    public function chiaSheng_update_text($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function chiaSheng_update_url($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function chiaSheng_update_textarea($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function chiaSheng_update_checkbox($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function chiaSheng_update_select($name, $new_instance)
    {
        $instance = array();
...
if (!function_exists('chiaSheng_get_cat_terms')):
function chiaSheng_get_cat_terms( $category_id = 0, $taxonomy='category', $default='' ){
    ...
    if ( $category_id > 0 ) {
        ...
        if (isset($terms) && !empty($terms)) {
            foreach ($terms as $term) {
                if( $default != 'first' ){
                    $array['0'] = __('Select Category', 'chiaSheng');
                }
...
if (!function_exists('chiaSheng_widget_crousel_column')):
function chiaSheng_widget_crousel_column( $default='1' ){
...
if ( ! function_exists( 'chiaSheng_custom_widgets' ) ) :
    /**
     * Load widgets
     */
    function chiaSheng_custom_widgets() {
        register_widget( 'chiaSheng_posts_slider_widget' );
        register_widget( 'chiaSheng_popular_post_widget');
    }
endif;
add_action( 'widgets_init', 'chiaSheng_custom_widgets' );



/**
 * chiaSheng Widgets - Loader.
 *
 * @package ChiaSheng Widget
 * @since 1.0.0
 */

if ( ! class_exists( 'chiaSheng_Widgets_Loader' ) ) {

    /**
     * Customizer Initialization
     */
    class chiaSheng_Widgets_Loader {
...
*/
chiaSheng_Widgets_Loader::get_instance();